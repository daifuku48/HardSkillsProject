...
class GetTextSecondInfoUseCase(
    ...
    override suspend fun execute(state: InfoSecondState, event: InfoSecondEvent): InfoSecondEvent {
        return if (event is InfoSecondEvent.GetTextEvent) {
            ...
            } else InfoSecondEvent.TextReceivedEvent(StateText.ErrorText)
        } else InfoSecondEvent.ErrorEvent
    }
