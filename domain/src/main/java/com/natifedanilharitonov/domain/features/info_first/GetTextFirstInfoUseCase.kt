...
class GetTextFirstInfoUseCase(
    ...
    override suspend fun execute(state: InfoFirstState, event: InfoFirstEvent): InfoFirstEvent {
        return if (event is InfoFirstEvent.GetTextEvent) {
            ...
            } else InfoFirstEvent.TextReceivedEvent(StateText.ErrorText)
        } else InfoFirstEvent.ErrorEvent
    }
